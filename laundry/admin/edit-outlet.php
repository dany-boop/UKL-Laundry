...
<section class="min-h-screen bg-yellow-50">
<div class="w-full lg:w-6/12 px-4 mx-auto pt-6">
  <div class="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-yellow-300 border-0">
    ...
    <?php  
          include ('koneksi.php');
          $qry_get=mysqli_query($conn,"select * from outlet where nama_outlet = '".$_GET['nama_outlet']."'");
          $dt_get=mysqli_fetch_array($qry_get);
          ?>  
      <form class="bg-yellow-300" action="../utils/proses-edit-outlet.php" method="post">
        <div class="relative w-full mb-3">
          <label class="block uppercase text-blueGray-600 text-xs font-bold mb-2" htmlfor="grid-password">Outlet Name</label>
          <input autocomplete="off" name="nama_outlet" type="text" class="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-yellow-100 rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" placeholder="Name"value="<?=$dt_get['nama_outlet']?>"/>
        </div>
        ...
        <div class="relative w-full mb-3">
          <label class="block uppercase text-blueGray-600 text-xs font-bold mb-2" htmlfor="grid-password">Phone</label>
