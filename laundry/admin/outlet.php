...
<div x-data="setup()" :class="{ 'dark': isDark }">
    <div class="min-h-screen flex flex-col flex-auto flex-shrink-0 antialiased bg-yellow-100 text-black dark:text-white">
      ...
      <div class="h-full ml-14 mt-14 mb-10 md:ml-64">
        ...
        <!-- Client Table -->
        <div class="mt-4 mx-4">
          <h1 class="text-xl text-black font-bold uppercase mb-5">Daftar Outlet</h1>
            ...
            <div class="w-full overflow-x-auto">
              <table class="w-full">
                ...
                <tbody class="bg-yellow-600 divide-y dark:divide-yellow-100">
                  <?php
                      include('koneksi.php');
                      $qry_karyawan = mysqli_query($conn,"select * from outlet");
                      $no=0;
                  ...
                  <tr class="hover:bg-yellow-700">
                    <td class="px-4 py-3">
                      <div class="flex items-center text-sm">
                        ...
                        <div>
                          <p class="font-semibold text-gray-50"><?=$data['nama_outlet'] ?></p>
                        </div>
                      </div>
                    </td>
                    <td class="px-4 py-3 text-sm text-gray-50"><?=$data['alamat'] ?></td>
                    <td class="px-4 py-3 text-xs"><?=$data['tlp'] ?></td>
                    <td class="px-4 py-3 text-sm">
                      <a href="edit-outlet.php?nama_outlet=<?=$data['nama_outlet'] ?>" class="px-4 py-2 text-xs rounded-full text-white bg-green-600 hover:bg-green-700"><i class="bi bi-pencil-square"></i> Ubah</a>
                      <a href="../utils/proses-delete-outlet.php?nama_outlet=<?=$data['nama_outlet'] ?>" class="px-4 py-2 text-xs rounded-full text-white bg-red-600 hover:bg-red-700" onclick="return confirm('Apakah anda yakin menghapus data ini?')"><i class="bi bi-trash"></i> Hapus</a>
                    </td>
                  </tr>
                  <?php } ?>
                </tbody>
